...
- commented the line 60 and changed a bit in line 253 of `reveal-code-focus.js`.

- There was some problem with source code line numbers. since a extra `span` with class `line` gets added, existing css for showing line number was not working. So changed that a bit `in custom.scss`.
